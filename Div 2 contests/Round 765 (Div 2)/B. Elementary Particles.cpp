...
void solve()
{
	...
	// find the longest string length containing 1 occurence of character in 2 subsegments
	for (auto &pr: pos)
	{
		ll sz=pr.ss.size();
		if (sz==1) continue;
		// greedy to take 2 adjacent occurence to maximize the string length
		for (ll i=0;i<sz-1;i++)
		{
			ll len=pr.ss[i]+(n-pr.ss[i+1]);
			ans=max(ans,len);
		}
	}
	cout<<ans;
