...
{
	// 3 types of lines
	ll line1=0,line2=0,line3=0;
	ans.pb(0);
	for (ll line=1;ans.back()<=1e9;line++)
	{
		ans.pb(ans.back());
		if (line%3==1)
		{
			ans.back()+=2*(line2+line3);
			line1++;
		}
		else if (line%3==2)
		{
			ans.back()+=2*(line1+line3);
			line2++;
		}
		else
		{
			ans.back()+=2*(line1+line2);
			line3++;
		}
	}
}
...
{
	ll n; cin>>n;
	ll res=lower_bound(all(ans),n)-ans.begin();
	cout<<res;
}
