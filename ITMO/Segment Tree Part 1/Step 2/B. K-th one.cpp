...
void update(ll ind, ll lo, ll hi, ll pos)
{
	if (lo>pos || hi<pos) return;
	if (lo!=hi)
	{
		ll mid=(hi+lo)/2;
		update(2*ind+1,lo,mid,pos);
		update(2*ind+2,mid+1,hi,pos);
		sgtree[ind]=sgtree[2*ind+1]+sgtree[2*ind+2];
	}
	else sgtree[ind]=1-sgtree[ind];
}

ll query(ll ind, ll lo, ll hi, ll k)
{
	if (lo==hi) return lo;
	else
	{
		ll mid=(hi+lo)/2;
		if (k<=sgtree[2*ind+1]) return query(2*ind+1,lo,mid,k);
		else return query(2*ind+2,mid+1,hi,k-sgtree[2*ind+1]);
	}
}
