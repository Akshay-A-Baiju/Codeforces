...
class SegTree
{
	private:
		...
		vll arr;
	public:
		SegTree(ll n) { arr.resize(4*n,df); }
		SegTree(ll n, ll _df) { df=_df; arr.resize(4*n,df); }
		ll merge(ll a, ll b) { return max(a,b); }
		void build(vll &v, ll ind, ll lo, ll hi)
		{
			if (lo==hi) { arr[ind]=v[lo]; return; }
			ll mid=(hi+lo)>>1;
		...
		void update(ll ind, ll lo, ll hi, ll pos, ll val)
		{
			...
			if (lo==hi) { arr[ind]=val; return; }
			ll mid=(hi+lo)>>1;
			update(2*ind+1,lo,mid,pos,val);
			update(2*ind+2,mid+1,hi,pos,val);
			arr[ind]=merge(arr[2*ind+1],arr[2*ind+2]);			
		}
		ll query(ll ind, ll lo, ll hi, ll x, ll l)
		{
			if (arr[ind]<x) return -1;
			if (lo==hi) return lo;
			ll mid=(hi+lo)/2;
			ll ans=-1;
			// check left segment
			if (arr[2*ind+1]>=x && mid>=l) ans=query(2*ind+1,lo,mid,x,l);
			if (ans!=-1) return ans;
			// if not found check right segment
			if (arr[2*ind+2]>=x && hi>=l) ans=query(2*ind+2,mid+1,hi,x,l);
			return ans;
		}
};
...
void solve()
{
	...
	while (m--)
	{
		...
		if (op==1)
		{
			...
			sg.update(0,0,n-1,pos,val);
		}
		else
		{
			ll x,l; cin>>x>>l;
			cout<<sg.query(0,0,n-1,x,l)<<endl;
		}
	}
}
