...
void build(vll &v, ll ind, ll lo, ll hi)
{
	if (lo==hi)
	{
		ll ans=max(0LL,v[lo]);
		sgtree[ind].sum=v[lo];
		sgtree[ind].ans=sgtree[ind].prf=sgtree[ind].suf=ans;
		return;
	}
	ll mid=(hi+lo)/2;
	build(v,2*ind+1,lo,mid);
	build(v,2*ind+2,mid+1,hi);
	sgtree[ind].sum=sgtree[2*ind+1].sum+sgtree[2*ind+2].sum;
	sgtree[ind].prf=max(sgtree[2*ind+1].prf,sgtree[2*ind+1].sum+sgtree[2*ind+2].prf);
	sgtree[ind].suf=max(sgtree[2*ind+2].suf,sgtree[2*ind+1].suf+sgtree[2*ind+2].sum);
	sgtree[ind].ans=max({sgtree[2*ind+1].ans,sgtree[2*ind+2].ans,sgtree[2*ind+1].suf+sgtree[2*ind+2].prf});
}
